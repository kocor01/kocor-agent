...
import pytest
from kocor.harness.permission import PermissionManager, TOOL_SAFETY_MAP, ToolSafetyLevel


class TestToolSafetyMap:
    def test_has_known_tools(self):
        assert "read_file" in TOOL_SAFETY_MAP
        assert "write_file" in TOOL_SAFETY_MAP
        assert "run_python" in TOOL_SAFETY_MAP

    def test_safety_levels(self):
        assert TOOL_SAFETY_MAP["read_file"].level == "caution"
        assert TOOL_SAFETY_MAP["write_file"].level == "dangerous"
        assert TOOL_SAFETY_MAP["run_python"].level == "dangerous"

    def test_unknown_tool_defaults_to_caution(self):
        assert TOOL_SAFETY_MAP.get("unknown_tool", ToolSafetyLevel("caution")).level == "caution"
...
class TestPermissionManager:
    ...
    def test_default_policy_allows_safe(self):
        ...
        assert pm.check("read_file") is False  # caution, no stdin -> denied

    def test_safe_tool_auto_allowed_in_default(self, monkeypatch):
        # Monkeypatch 安全等级为 "safe"
        pm = PermissionManager(policy="default")
        assert pm.check("read_file") is False  # no stdin -> denied for caution
    ...
    def test_always_ask_denies_without_stdin(self):
        ...
        assert pm.check("write_file") is False
