...
@patch("kocor.llm_provider.llm_factory.LlmFactory.create", return_value=FakeLLMForSummary())
class TestSlidingWindowStrategy:
    ...
    def test_truncates_old_rounds(self, mock_create):
        """超出保留轮次时，旧轮次被摘要。"""
        all_msgs = []
        for i in range(5):
    ...
    def test_preserves_latest_rounds(self, mock_create):
        """最近的轮次应完整保留。"""
        all_msgs = []
        for i in range(4):
            all_msgs.extend(make_round(f"问题{i}", f"回答{i}"))
    ...
    def test_aggressive_mode(self, mock_create):
        """AGGRESSIVE 策略（preserve_last=1）只保留最后 1 轮（即最后的 assistant 输出）。"""
        strategy = SlidingWindowStrategy(
            preserve_last_rounds=1, preserve_first_rounds=0,
        )
        all_msgs = []
        for i in range(4):
            all_msgs.extend(make_round(f"问题{i}", f"回答{i}"))
    ...
    def test_empty_messages(self, mock_create):
        """空消息列表应返回空。"""
        result, summary = self.strategy.apply([])
        assert result == []
        assert summary is None

    def test_single_round(self, mock_create):
        """只有一轮时不截断。"""
    ...
    def test_round_with_tool_calls(self, mock_create):
    ...
    def test_preserve_first_rounds_keeps_initial_rounds(self, mock_create):
        ...
        strategy = SlidingWindowStrategy(
            preserve_last_rounds=2, preserve_first_rounds=2,
        )
        all_msgs = []
        for i in range(6):
            all_msgs.extend(make_round(f"问题{i}", f"回答{i}"))
    ...
    def test_first_plus_last_exceeds_total_no_summary(self, mock_create):
        """preserve_first + preserve_last >= 总轮次时不应摘要。"""
        strategy = SlidingWindowStrategy(
            preserve_last_rounds=2, preserve_first_rounds=3,
        )
        all_msgs = []
        for i in range(2):
            all_msgs.extend(make_round(f"问题{i}", f"回答{i}"))
    ...
    def test_preserve_first_rounds_zero_backward_compat(self, mock_create):
    ...
    def test_preserve_first_middle_empty_no_summary(self, mock_create):
    ...
    def test_preserve_first_keeps_correct_order(self, mock_create):
