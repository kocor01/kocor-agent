...
class TestJobsCRUD:
    ...
    def test_create_job_minimal(self):
        ...
        job = create_job(prompt="say hello", schedule="2099-01-01T00:00:00")
    ...
    def test_create_job_cron(self):
        """创建 cron 重复作业。"""
        job = create_job(prompt="check status", schedule="*/10 * * * *")
        assert job["schedule"]["kind"] == "cron"
        assert job["schedule"]["expr"] == "*/10 * * * *"
        assert job["repeat"] == {"times": None, "completed": 0}

    def test_create_job_with_name(self):
        """创建命名作业。"""
        job = create_job(prompt="daily report", schedule="0 * * * *", name="Daily Report")
    ...
    def test_create_job_auto_repeat_once_for_oneshot(self):
        """一次性调度自动设置 repeat=1。"""
        job = create_job(prompt="one time", schedule="2099-01-01T00:00:00")
        assert job["repeat"] == {"times": 1, "completed": 0}

    def test_create_job_with_repeat(self):
        """指定重复次数。"""
        job = create_job(prompt="run 5 times", schedule="*/10 * * * *", repeat=5)
        assert job["repeat"] == {"times": 5, "completed": 0}

    def test_get_job_by_id(self):
        """按 ID 获取作业。"""
        created = create_job(prompt="test", schedule="2099-01-01T00:00:00")
    ...
    def test_resolve_job_ref_by_name(self):
        """按名称解析引用。"""
        created = create_job(prompt="test name resolve", schedule="2099-01-01T00:00:00", name="UniqueName")
    ...
    def test_resolve_job_ref_ambiguous(self):
        ...
        create_job(prompt="first", schedule="2099-01-01T00:00:00", name="SameName")
        create_job(prompt="second", schedule="2099-01-01T00:00:00", name="SameName")
    ...
    def test_list_jobs_exclude_disabled(self):
        """list_jobs 默认排除已禁用作业。"""
        job = create_job(prompt="to disable", schedule="2099-01-01T00:00:00")
    ...
    def test_list_jobs_include_disabled(self):
        ...
        job = create_job(prompt="disabled job", schedule="2099-01-01T00:00:00")
        update_job(job["id"], {"enabled": False, "state": "paused"})
    ...
    def test_remove_job(self):
        """删除作业。"""
        created = create_job(prompt="to delete", schedule="2099-01-01T00:00:00")
    ...
    def test_update_job_prompt(self):
        """更新作业 prompt。"""
        job = create_job(prompt="original", schedule="2099-01-01T00:00:00")
...
class TestJobsStorage:
    ...
    def test_persistence_across_load(self, cron_dir: Path):
        """创建后文件写入磁盘，可重新加载。"""
        job = create_job(prompt="persist test", schedule="2099-01-01T00:00:00")
    ...
    def test_jobs_file_structure(self, cron_dir: Path):
        """验证 JSON 文件结构。"""
        create_job(prompt="test", schedule="2099-01-01T00:00:00")
        jobs_file = cron_dir / "jobs.json"
    ...
    def test_remove_cleans_output_dir(self, cron_dir: Path):
        """删除作业同时清理输出目录。"""
        job = create_job(prompt="cleanup test", schedule="2099-01-01T00:00:00")
...
class TestJobsEdgeCases:
    ...
    def test_create_job_with_empty_prompt_fails(self):
        ...
        with pytest.raises(ValueError, match="prompt or at least one skill"):
            create_job(prompt="", schedule="2099-01-01T00:00:00")

    def test_create_job_with_skills_only(self):
        """仅 skills 也能创建作业。"""
        job = create_job(prompt="", schedule="*/10 * * * *", skills=["greeter"])
        assert job["skills"] == ["greeter"]

    def test_create_job_with_zero_repeat(self):
        """repeat=0 视为无限（None）。"""
        job = create_job(prompt="test", schedule="*/10 * * * *", repeat=0)
        assert job["repeat"]["times"] is None

    def test_create_job_with_negative_repeat(self):
        """repeat 负数视为无限。"""
        job = create_job(prompt="test", schedule="*/10 * * * *", repeat=-1)
        assert job["repeat"]["times"] is None
