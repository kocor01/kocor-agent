...
class Logs:
    ...
    def _handle_pre_generate(self, event: HarnessEvent) -> None:
        get_logger().event(EventType.PRE_GENERATE, logging.DEBUG, **event.data)

    def _handle_post_generate(self, event: HarnessEvent) -> None:
        get_logger().event(EventType.POST_GENERATE, logging.DEBUG, **event.data)

    def _handle_pre_tool(self, event: HarnessEvent) -> None:
        get_logger().event(EventType.PRE_TOOL, logging.DEBUG, **event.data)

    def _handle_post_tool(self, event: HarnessEvent) -> None:
        get_logger().event(EventType.POST_TOOL, logging.DEBUG, **event.data)

    def _handle_on_error(self, event: HarnessEvent) -> None:
        get_logger().event(EventType.ON_ERROR, logging.ERROR, **event.data)

    def _handle_on_budget_exhausted(self, event: HarnessEvent) -> None:
        get_logger().event(EventType.ON_BUDGET_EXHAUSTED, logging.WARNING, **event.data)
