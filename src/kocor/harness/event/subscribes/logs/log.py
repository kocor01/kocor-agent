...
class Logs:
    ...
    def __init__(self, logger: HarnessLogger):
        self._logger = logger

    def _handle_pre_generate(self, event: HarnessEvent) -> None:
        self._logger.event(EventType.PRE_GENERATE, **event.data)

    def _handle_post_generate(self, event: HarnessEvent) -> None:
        self._logger.event(EventType.POST_GENERATE, **event.data)

    def _handle_pre_tool(self, event: HarnessEvent) -> None:
        self._logger.event(EventType.PRE_TOOL, **event.data)

    def _handle_post_tool(self, event: HarnessEvent) -> None:
        self._logger.event(EventType.POST_TOOL, **event.data)

    def _handle_on_error(self, event: HarnessEvent) -> None:
        self._logger.event(EventType.ON_ERROR, **event.data)

    def _handle_on_budget_exhausted(self, event: HarnessEvent) -> None:
        self._logger.event(EventType.ON_BUDGET_EXHAUSTED, **event.data)